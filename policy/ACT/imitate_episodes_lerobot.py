"""
ACT policy training script using LeRobotDataset-format data.

This is the LeRobot-based counterpart of imitate_episodes.py. It reads data
from a LeRobotDataset (produced by process_data_lerobot.py) and trains the
ACT policy with the same training loop.

Usage:
    python imitate_episodes_lerobot.py \
        --repo_id act_task_config_50 \
        --ckpt_dir ./ckpts/act_lerobot \
        --policy_class ACT \
        --batch_size 8 \
        --seed 0 \
        --num_epochs 2000 \
...
from utils_lerobot import load_data_lerobot
...
def main(args):
    ...
    if is_eval:
        print("Evaluation mode is not supported in LeRobot training script.")
        print("Use the original imitate_episodes.py for evaluation.")
        exit()

    # Load data from LeRobotDataset
    train_dataloader, val_dataloader, stats = load_data_lerobot(
        repo_id, camera_names, chunk_size, batch_size_train, batch_size_val,
    )
