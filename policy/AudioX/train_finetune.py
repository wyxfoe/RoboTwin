...
Usage:
    ...
    # Multi-task fine-tune with weighted sampling:
    python train_finetune.py \
        --config configs/robotx_finetune_1_2b.json \
        --audiox_ckpt /path/to/audiox_1.2b.ckpt \
    ...
    # Multi-task with custom per-task weights:
    python train_finetune.py \
        --config configs/robotx_finetune_1_2b.json \
        --audiox_ckpt /path/to/audiox_1.2b.ckpt \
        --task /path/to/task_a/data "task a" \
        --task /path/to/task_b/data "task b" \
        --sampling_strategy custom --task_weights 2.0 1.0
...
def main():
    ...
    parser.add_argument("--sampling_strategy", type=str, default="uniform",
                        choices=["uniform", "sqrt", "natural", "custom"],
                        help="Task sampling strategy for cross-task transfer: "
    ...
    # Build task_dirs from --task arguments or fall back to config / --data_dir
    task_dirs = None
    if args.task:
